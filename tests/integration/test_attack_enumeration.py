...
from tests.integration.utils.data import archer_template, goblin_template, simple_mission
...
def test_archer_surrounded_attack_count_and_perf(base_url: str):
    ...
    max_rng = max(abs(x - center[0]) + abs(y - center[1]) for x in range(width) for y in range(height))
    rng = max_rng
    archer.stats.base[StatName.RNG] = rng
    enemy_positions = [(x, y) for x in range(width) for y in range(height) if (x, y) != center]
    ...
    r = requests.get(f"{base_url}/sessions/{sid}/legal_actions", params={"explain": "true"}, timeout=5)
    r.raise_for_status()
    actions = r.json()["actions"]
    attack_actions = [a for a in actions if a.get("action", {}).get("kind") == "ATTACK"]
    # All enemies should be in range now
    assert len(attack_actions) == len(enemies), (
    ...
    for _ in range(N):
        ...
        rr = requests.get(f"{base_url}/sessions/{sid}/legal_actions", params={"explain": "true"}, timeout=5)
        rr.raise_for_status()
        _ = rr.json()
    ...
    logger.info(
        ...
        sid, N, avg_ms, len(enemies),
    )
    assert avg_ms < 20, f"average legal_actions response {avg_ms:.2f} ms exceeds 17 ms threshold"
